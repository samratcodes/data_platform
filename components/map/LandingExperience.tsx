  ...
  return (
    <main className={`sourcing-app landing-experience ${directoryMinimized ? "directory-collapsed" : ""}`}>
      ...
      <section ref={mapSection} className="landing-map-section" aria-label="Explore the map">
        ...
        {query.trim() && <div className="landing-quick-results" aria-label="Search results"><div><span>Verified network</span><strong>{filtered.length} {filtered.length === 1 ? "match" : "matches"}</strong></div>{filtered.slice(0, 4).map((operator) => <button key={operator.slug} onClick={() => { setGatedOperator(operator); map?.flyTo(operator.coordinates, 8); }}><ProviderLogo cover={Boolean(operator.profile?.logoIsPhoto)} name={operator.name} logo={operator.profile?.logo} type={operator.type} size={38}/><span><strong>{operator.name}</strong><small>{operator.city}, {operator.country}{operator.company ? ` · by ${operator.company.name}` : ""} · {operator.modalities.slice(0, 2).join(" + ")}</small></span><BadgeCheck size={16}/><ArrowUpRight size={15}/></button>)}{filtered.length === 0 && <div className="landing-quick-empty"><Search size={18}/><span><strong>No network match</strong><small>Try a city, company, or modality.</small></span></div>}</div>}
        ...
        <AnimatePresence initial={false}>{!directoryMinimized && <motion.aside id="public-provider-directory" className="map-company-list" aria-label="Verified provider directory" initial={reducedMotion ? false : { opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={reducedMotion ? { opacity: 0 } : { opacity: 0, x: 12 }} transition={{ duration: reducedMotion ? 0 : .22, ease: [.22, 1, .36, 1] }}>
          ...
          <div className="map-company-list-scroll">{filtered.map((operator) => <button key={operator.slug} className="has-logo" onClick={() => { setGatedOperator(operator); map?.flyTo(operator.coordinates, 7); }}><ProviderLogo cover={Boolean(operator.profile?.logoIsPhoto)} name={operator.name} logo={operator.profile?.logo} type={operator.type} size={38}/><strong><VerificationMark level={operator.verificationLevel}/>{operator.name}</strong><small>{operator.city}, {operator.country}{operator.company && <> · by {operator.company.name}</>}</small><em>{operator.modalities.slice(0, 2).join(" · ")}</em></button>)}</div>
      ...
      {gatedOperator && <Modal title="Provider preview" onClose={() => setGatedOperator(null)}><div className="public-provider-preview"><div className="public-provider-photo"><Image src={gatedOperator.media.src} alt={gatedOperator.media.alt} fill sizes="(max-width: 640px) 90vw, 520px"/></div><span className="verified-status"><BadgeCheck size={13}/>{gatedOperator.verificationLevel === "physical" ? "Physically verified" : "Online verified"}</span><h3>{gatedOperator.name}</h3><p className="public-provider-location"><MapPin size={15}/>{gatedOperator.city}, {gatedOperator.country}</p>{gatedOperator.company && <p className="provider-company-line">Facility of <strong>{gatedOperator.company.name}</strong></p>}<p>Log in to view capabilities, samples, operating details, and contact options.</p><div className="public-provider-actions"><Link className="primary-button" href={`/login?next=${encodeURIComponent(`/map?operator=${gatedOperator.slug}`)}`}><LogIn size={16}/>Log in to view</Link></div></div></Modal>}
    </main>
  );
}
