...
export default function AuthForm({ signup = false, defaultRole = "buyer", lockRole = false }: { signup?: boolean; defaultRole?: "buyer" | "supplier"; lockRole?: boolean }) {
  ...
  return <main className={`sourcing-app auth-page ${signup ? "signup-page" : "login-page"}`}>
    <PublicNavigationRail active={signup && defaultRole === "supplier" ? "supplier-signup" : signup ? undefined : "login"}/>
      ...
      <motion.section initial={reducedMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="glass auth-card">
        ...
        <form onSubmit={async (event) => {
          ...
          try {
            ...
            if (signedInRole !== "admin" && !emailVerifiedAt) router.push("/verify-email");
            else if (next && canAccess(next.split(/[?#]/)[0], signedInRole)) router.push(next);
            else router.push(signup && signedInRole === "supplier" ? "/onboarding" : homePathFor(signedInRole));
            router.refresh();
