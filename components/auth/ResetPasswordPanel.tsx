  ...
  return <main className="sourcing-app auth-page">
    ...
    <section className="auth-layout compact-auth">
      <form className="glass auth-card recovery-card" onSubmit={async (event) => {
        ...
        try {
          const result = await api<{ user?: { role: UserRole; emailVerifiedAt?: string | null } | null }>("/api/auth/reset-password", { method: "POST", body: JSON.stringify({ ...values, token }) });
          setDone(true);
          const href = !result.user ? "/login" : result.user.role === "admin" || result.user.emailVerifiedAt ? homePathFor(result.user.role) : "/verify-email";
