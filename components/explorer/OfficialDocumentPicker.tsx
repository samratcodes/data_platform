...
export default function OfficialDocumentPicker({ documents, onChange, onError }: {
  ...
  return <section className="official-document-picker" aria-label="Official company documents">
    ...
    {documents.length ? <div className="official-document-gallery">{documents.map((document, index) => <article key={document.url}><button type="button" className="official-document-thumbnail" onClick={() => setPreviewUrl(document.url)} aria-label={`Preview document ${index + 1}: ${document.name}`}>{document.contentType === "application/pdf" ? <FileText size={40}/> : <Image src={document.url} alt={document.name} fill unoptimized sizes="150px"/>}<span><Eye size={14}/>Preview</span></button><div className="official-document-details"><small>{document.name} · {sizeLabel(document.file.size)}</small><label>Document type<input value={document.type} maxLength={120} onChange={(event) => updateType(document.url, event.target.value)} placeholder="e.g. Certificate of incorporation" aria-label={`Document type for ${document.name}`}/></label><button type="button" onClick={() => remove(document.url)} aria-label={`Delete ${document.name}`}><Trash2 size={15}/>Delete</button></div></article>)}</div> : <p className="official-document-empty">Selected documents will appear here. You can preview each one and name its document type.</p>}
    {preview && <div className="official-document-preview-backdrop" role="presentation" onClick={() => setPreviewUrl(null)}><div className="official-document-preview" role="dialog" aria-modal="true" aria-label={`Preview ${preview.name}`} onClick={(event) => event.stopPropagation()}><header><div><small>OFFICIAL DOCUMENT {documents.indexOf(preview) + 1} OF {documents.length}</small><strong>{preview.name}</strong></div><button type="button" aria-label="Close document preview" onClick={() => setPreviewUrl(null)}><X size={18}/></button></header>{preview.contentType === "application/pdf" ? <iframe src={preview.url} title={preview.name}/> : <div className="official-document-preview-image"><Image src={preview.url} alt={preview.name} fill unoptimized sizes="(max-width: 800px) 95vw, 750px"/></div>}<footer><label>Document type<input value={preview.type} maxLength={120} onChange={(event) => updateType(preview.url, event.target.value)} placeholder="What type of document is this?"/></label><button type="button" onClick={() => remove(preview.url)}><Trash2 size={15}/>Delete</button><button type="button" className="official-document-preview-done" onClick={() => setPreviewUrl(null)}>Done</button></footer></div></div>}
  </section>;
}
