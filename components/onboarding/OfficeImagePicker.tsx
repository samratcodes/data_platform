...
export type OfficeImage = { url: string; name: string; contentType: string; file: File };
...
export default function OfficeImagePicker({ images, onChange, onError }: {
  images: OfficeImage[];
  ...
  const selectFiles = (fileList: FileList | null) => {
    ...
    onError("");
    const selection = files.map((file) => { const url = URL.createObjectURL(file); previewUrls.current.add(url); return { file, url, name: file.name, contentType: file.type }; });
  ...
  return <section className={`office-image-picker ${dragging ? "is-dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); selectFiles(event.dataTransfer.files); }}>
    <div className="office-image-picker-heading"><span className="wizard-upload-icon"><ImagePlus/></span><div><strong>Office and company images <em>Optional</em></strong><small>JPG, PNG, or WebP · up to 10 images · 50 MB total</small></div></div>
    ...
    {images.length ? <div className="office-image-gallery" aria-label="Selected office images">{images.map((image, index) => <article key={image.url}><button type="button" className="office-image-gallery-view" onClick={() => { setPending([]); setActiveUrl(image.url); }} aria-label={`View image ${index + 1}: ${image.name}`}><Image src={image.url} alt={image.name} fill unoptimized sizes="160px"/><span><Eye size={15}/>View</span></button><div><strong>{image.name}</strong><small>{sizeLabel(image.file.size)}</small><button type="button" onClick={() => remove(image.url)} aria-label={`Delete ${image.name}`}><Trash2 size={16}/></button></div></article>)}</div> : <p className="office-image-picker-empty">Your images will appear here after you review and confirm them.</p>}
    {pending && <div className="office-image-review-backdrop" role="presentation" onClick={closeReview}><div className="office-image-review" role="dialog" aria-modal="true" aria-label="Review office images" onClick={(event) => event.stopPropagation()}><header><div><small>IMAGE REVIEW</small><h3>Review all selected images</h3><p>{reviewImages.length} of 10 images · {sizeLabel(reviewImages.reduce((sum, image) => sum + image.file.size, 0))} of 50 MB</p></div><button type="button" aria-label="Close image review" onClick={closeReview}><X size={19}/></button></header><div className="office-image-review-main">{active && <div className="office-image-review-hero"><Image src={active.url} alt={active.name} fill unoptimized sizes="(max-width: 800px) 90vw, 600px"/><span>{active.name}</span></div>}<div className="office-image-review-grid">{reviewImages.map((image, index) => <article key={image.url} className={active?.url === image.url ? "active" : ""}><button type="button" onClick={() => setActiveUrl(image.url)} aria-label={`Preview image ${index + 1}: ${image.name}`}><Image src={image.url} alt={image.name} fill unoptimized sizes="120px"/></button><small>{image.name}</small><button type="button" className="office-image-review-delete" onClick={() => remove(image.url)} aria-label={`Delete ${image.name}`}><Trash2 size={14}/></button></article>)}</div></div><footer><button type="button" onClick={closeReview}>Cancel</button><button type="button" className="office-image-review-confirm" onClick={() => { onChange([...images, ...pending]); setPending(null); setActiveUrl(null); }} disabled={!pending.length}>{pending.length ? `OK · keep ${pending.length} new image${pending.length === 1 ? "" : "s"}` : "Done"}</button></footer></div></div>}
  </section>;
}
