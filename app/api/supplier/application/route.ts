...
export async function POST(request: Request) {
  ...
  try {
      ...
      if (existing) {
        await client.query(`
          ...
          SET business_name = $1, profile_description = $2, website_url = $3,
              ...
              company_focus = $22, verification_level = 'unverified', admin_notes = NULL, reviewed_at = NULL,
              submitted_at = NOW(),
        ...
        `, [businessName, description, website, safeHttpsUrl(body.linkedinUrl), safeHttpsUrl(body.twitterUrl), safeHttpsUrl(body.huggingFaceUrl), mapsUrl, address, city, country, longitude, latitude, JSON.stringify(selectedModalities), JSON.stringify(companyPictures), validSample, sampleName || null, validSample ? sampleType : null, validSample ? sampleBuffer?.length : null, validSample ? sampleBuffer : null, id, user.id, focus]);
      } else {
        await client.query(`
          INSERT INTO supplier_applications (
            ...
            sample_size_bytes, sample_data, company_focus
    ...
    const city = cleanSingleLine(body.city, 100);
    const country = cleanSingleLine(body.country, 100);
    const details = parseFacilityDetails(body.facility);
    const longitude = Number(body.longitude);
    const latitude = Number(body.latitude);
    const mapsUrl = safeHttpsUrl(body.mapsUrl);
    ...
    if (address.length < 5 || city.length < 2 || country.length < 2 || !mapsUrl || !isGoogleMapsUrl(mapsUrl) || !Number.isFinite(longitude) || !Number.isFinite(latitude) || longitude < -180 || longitude > 180 || latitude < -90 || latitude > 90) {
      await client.query("ROLLBACK");
      return Response.json({ error: "Complete the facility location: pin it on the map and confirm its address, city, and country." }, { status: 400 });
    }
    const capacity = facilityCapacity(details);

    // Editing an existing factory: the approved listing stays live until the revision is approved.
    if (body.applicationId !== undefined) {
      const applicationId = isUuid(body.applicationId) ? body.applicationId : "";
      const updated = applicationId && await client.query(`
        UPDATE supplier_applications
        SET business_name = $1, profile_description = $2, maps_url = $3, physical_address = $4, city = $5, country = $6,
            longitude = $7, latitude = $8, modalities = $9::jsonb, capacity = $10, capture_environments = $11::jsonb,
            facility_details = $12::jsonb,
            hardware_pictures = CASE WHEN $13::boolean THEN $14::jsonb ELSE hardware_pictures END,
            status = 'pending', verification_level = 'unverified', admin_notes = NULL, reviewed_at = NULL, submitted_at = NOW()
        WHERE id = $15 AND user_id = $16 AND application_kind = 'facility'
      `, [businessName, description, mapsUrl, address, city, country, longitude, latitude, JSON.stringify(selectedModalities), capacity, JSON.stringify(environments), JSON.stringify(details), Array.isArray(body.hardwarePictures), JSON.stringify(pictures), applicationId, user.id]);
      if (!updated || !updated.rowCount) {
        await client.query("ROLLBACK");
        return Response.json({ error: "The facility could not be found." }, { status: 404 });
      }
      await enqueueUserSheetSync(user.id, client);
      await client.query("COMMIT");
      return Response.json({ id: applicationId, kind, status: "pending" });
    }
    ...
    await client.query(`
      INSERT INTO supplier_applications (
        ...
        capacity, capture_environments, provider_slug, facility_details
      ) VALUES ($1,$2,'facility',$3,$4,$5,$6,$7,$8,$9,$10::jsonb,'Facility',$11::jsonb,$12,$13,$14::jsonb,$15,$16::jsonb)
    `, [id, user.id, businessName, mapsUrl, address, city, country, longitude, latitude, JSON.stringify(pictures), JSON.stringify(selectedModalities), description, capacity, JSON.stringify(environments), providerSlug, JSON.stringify(details)]);
    await enqueueUserSheetSync(user.id, client);
    await client.query("COMMIT");
